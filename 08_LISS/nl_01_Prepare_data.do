...
/* 
================================================================================
LISS Data Preparation Description - nl_01_Prepare_data.do
================================================================================

The nl_01_Prepare_data.do file processes data from the LISS background 
variables and topic-specific data files, ultimately producing a harmonized 
dataset for CPF. The process includes the following steps:

Setup and Directories:
• Defines input and output directories
• Creates temporary folders for processing
• Installs necessary Stata packages (fs for file scanning)
...
• Appends all monthly background variable files into one (large) file: 
  liss01_bckgr_all.dta

1c. Compute Incomes from All Months
• Computes income aggregates from all months to be used in the main file 
  during harmonization
• Calculates for each person-year:
  - Number of months with income information for variables: brutoink, 
    nettoink, brutoink_f, netinc, nettoink_f, brutohh_f, nettohh_f
  - Sum of income from all months within each year
• Creates variables with suffixes _count and _sum for later use in 
  harmonization

1d. Select Reference Months
• Selects reference months per year and generates the main background 
  file for CPF integration with reference months
...
• Implements backup selection for cases without month 4 data (selects 
  closest available month)
• Handles different data collection periods across waves (2007-2014, 
  2015-2016, 2017+)
• Creates the final reference file: liss01_bckgr_ref.dta (contains only 
  1 observation per person per wave)
...
• Data collection period for each LISS wave is spread over many months 
  and has been changing over the years. Some topics (e.g. Health, Politics) 
  were collected over two separate calendar years, and additionally, the 
  period has been significantly changing. Thus, some decisions about 
  assigning particular topic's wave to the general LISS wave are not 
  straightforward. Please, consult the LISS data collection table (in CPF 
  materials) for an overview of the LISS waves' design for CPF integration.
• Implements special handling for:
  - Topic 02 (Health): Wave-specific year assignments; skipped wave 9 (2016)
  - Topic 08 (Politics): Wave-specific year assignments; skipped Wave 10 
    (2017) dues to modification in the LISS data collection schedule. New 
    data were combined with next wave 11
  - Topic 09 (Economic Situation: Assets): Biennial collection handling
• Creates standardized variables:
  - intdate, intyear, intmonth
  - wavey (main year of data collection)
  - fileorder (sequence within topic)
• Output files: in "temp/'topic'/…" for each topic and wave (e.g., 
  liss02_08 – topic 2 wave 8)
...
-----------------------------------------
• Appends waves within each topic
• Preserves all variables
• Output files: temp/topics/liss'topic', e.g., liss02 – topic 2 with all 
  waves

Step 4: Merge All Topics
------------------------
• Creates a combined dataset for topics 02-11 (without background variables 
  yet): liss01_all_topics.dta
• Ensures respondent-wave consistency across all topics

Step 5: Final Dataset Creation - Merge Background & Topics
----------------------------------------------------------
• Merges the background file (liss01_bckgr_ref.dta) with the topics file 
  (liss01_all_topics.dta)
• Keeps only matched cases (participants with background variables)
• Creates final CPF-LISS dataset: nl_01.dta
...
capture confirm variable cv16h_m
if _rc {
    gen cv16h_m = maandnr
}
...
forvalues year = 18/24  {
    ...
    * Check if variable exists before dropping
    capture confirm variable cv`year'_m
    if !_rc {
        drop cv`year'_m
    }
    capture confirm variable cvx`year'_m
    if _rc {
...
local files : dir "${liss_out}/temp" files "*.dta"
foreach f of local files {
    erase "${liss_out}/temp/`f'"
}
* Also remove all temporary .dta files in sub-folders of the temp directory
local subfolders : dir "${liss_out}/temp" dirs "*"
foreach subf of local subfolders {
    local files : dir "${liss_out}/temp/`subf'" files "*.dta"
    foreach f of local files {
        erase "${liss_out}/temp/`subf'/`f'"
    }
}
