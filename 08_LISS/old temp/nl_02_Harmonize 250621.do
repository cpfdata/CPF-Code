...
*** Directories 
global liss_out "C:/Users/klturek/OneDrive - Tilburg University/_KT_work/_CPF/__LISS/cpf_out"
...
* use "${liss_out}/nl_01_23.dta", clear
use "${liss_out}/nl_01.dta", clear
use "${liss_out}/nl_01_working.dta", clear
...
***TO DO:
!!!! check othe surveys: if previous year, prev. 12 m, current year

* bro pid wavey brutoink brutoink_f nettoink nettoink_f brutoink_count nettoink_count brutoink_sum nettoink_sum netinc_sum
* bro pid wavey  nettoink nettoink_count nettoink_f nettoink_f_count nettoink_sum nettoink_f_sum netinc_sum netinc_count
...
// Last callendar year (lagged varaible), based on background variables (nettoink_sum & nettoink_count)
// Only if information availible for all 12 months of the previous year
// If <12 months of information --> MV 

gen incjobs_yn_prevyALLm= .
bysort pid (wavey): replace incjobs_yn_prevyALLm=nettoink_f_sum[_n-1] if nettoink_f_count[_n-1]==12
bysort pid (wavey): replace incjobs_yn_prevyALLm=-9 if nettoink_f_count[_n-1]<12 & nettoink_f_count[_n-1]>0 // some information avaliable

gen incjobs_yg_prevyALLm= .
bysort pid (wavey): replace incjobs_yg_prevyALLm=brutoink_f_sum[_n-1] if brutoink_f_count[_n-1]==12
bysort pid (wavey): replace incjobs_yg_prevyALLm=-9 if brutoink_f_count[_n-1]<12 & brutoink_f_count[_n-1]>0 // some information avaliable


 gen incjobs_yn= incjobs_yn_prevyALLm // previous callendar year!
 gen incjobs_yg= incjobs_yg_prevyALLm // previous callendar year!

	lab var incjobs_yn "Individual Labor Earnings (All jobs, prev cal year, net)"
	lab var incjobs_yg "Individual Labor Earnings (All jobs, prev cal year, gross)"
...
* 1. Takes value from HH box (imputed) for the reference month for backgroung varialbes
  gen hhinc_pre=brutohh_f  
  gen hhinc_post=nettohh_f   
...
* Average (from entire current year)
 ...
 replace hhinc_pre=hhinc_b_average if hhinc_pre==. & hhinc_b_average<. & hhinc_b_average>0
 replace hhinc_post=hhinc_n_average if hhinc_post==. & hhinc_n_average<. & hhinc_n_average>0
...
label data "CPF_Netherlands v2.0"	 
