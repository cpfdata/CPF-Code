...
Syntax 02: Harmonize
===============================================================================
Purpose: Harmonize UKHLS/BHPS panel data files across all waves
Author:  Konrad Turek
Date:    06.2025
Input:   uk_01.dta (combined person-household file)
Output:  uk_02.dta (harmonized person-household file)
===============================================================================
*/
...
5. EMPLOYMENT 		
6. INCOME AND ECONOMIC 
7. HEALTH 		
8. SATISFACTION 	
9. WEIGHTS AND SAMPLE IDENTIFIERS
...
* Log
capture log close 
log using "${ukhls_out}/uk_02_harmonization.log", replace
display "Starting UKHLS data harmonization at $S_TIME"
...
/***** Identify resp with repeated age - however, repeated age by wave may happen in the UK survey design 
...
* Gender
* NOTE: 'Other' gender is not identified in the UKHLS
recode sex (-9/-1=2) (1=0) (2=1), gen(female)
...
***
/* General education recoding
==== Primary education variables
isced // main for waves 1-18
qfhigh_dv // main for waves 19+ 

==== Special samples  w 24, 27 (outside UK)
qfhighoth (isced11_dv) 
...
// mastat mlstat_bh // waves 1-18
// marstat marstat_dv mastat_dv // waves 19+ --> CNEF: mastat_dv 
// mlstat // all waves 
...
**--------------------------------------
/* Approach based on CNEF. Equal to CNEF's d11104  
NOTE: 
...
*################################################################################
*# 
*# Labour market situation: basic variables
*# 
*################################################################################	
...
recode jbstat 	(1 2 5 10 11 12 13 14 15=1) (3=2) (4 8=3) (6=4) (7 9=5) ///
...
* emplst6
recode jbstat 	(1 2 10 11=1) (3=2) (4 8=3) (6=4) (7 9=5) (5 12 13 14 15=6) ///
...
*################################################################################
*# 
*# Labour market situation: occupation and industry
*# 
*################################################################################	
...
recode  indust2d2 (1/49=1)(50/74 90/96=2)(75/86 89 99=3) (.=-1) ///
				   , gen(indust1b)				   
				   
recode  indust2d3 (1/43=1)(45/82 87 90 92/98=2)(84/86 88 91 99=3) (.=-1) ///
				   , gen(indust1c)						   
...
recode  indust2d3 (1/3=1)(4=2)(5/9=3)(10/33=4)(35/37=5) ///
				  ...
				  (62 64/68 72/75 77=11)(84=12)(38 39 78 91 93 94 96=15)(85=13)(75 86/88=14)(.=-1)	///
				  (97=16)(99=17) ///
				   , gen(indust3c)
gen indust3=.
...
*################################################################################
*# 
*# Labour market situation: sector, size, hours, etc.
*# 
*################################################################################	
...
**--------------------------------------
* mater
recode jbstat (5 14 15 =1) (1/4 6/97=0) (-9 -2 -1=-1) (-8 -7=-3) , gen(mater)
...
/* retf - Critetia for definition of retired:
- Not working  
...
*** all jobs 
// fiyr fimngrs_dv - Labour income: last month  
recode fimnlabgrs_dv (min/-0.0001=.) , gen(incjobs_mg)

* Estimated annual income for the current year based on monthly income
gen incjobs_cyg=incjobs_mg*12
 

	lab var incjobs_cyg "Individual Labor Earnings (All jobs, current year-estimated, gross)"
...
* Estimated annual income for the current year based on monthly income
gen incjob1_cyg=incjob1_mg*12
gen incjob1_cyn=incjob1_mn*12

	lab var incjob1_cyg  "Salary from main job (current year-estimated, gross)"
...
recode fihhmngrs_dv   (-9=.), gen(hhinc_pypre)
recode fihhmnnet1_dv  (-9=.), gen(hhinc_pypost)

		lab var hhinc_pypre "HH income(prev. year, pre-tax)"	
	 	lab var hhinc_pypost "HH income(prev. year, post-tax)"	
...
recode kidsn_hh15 (min/0=0), gen(temp_kidsn_hh15)

* Check if the number of people in the household is consistent with the number of children aged 0-17
gen temp_err = temp_nphh-temp_kidsn_hh15
recode temp_err (min/-1=-1) (0/20=1)
tab temp_err

*** Equivalized HH income
gen hhinc_pypost_eq = temp_hhinc_pypost/(1 + .5*(temp_nphh - temp_kidsn_hh15 - 1) + .3*temp_kidsn_hh15) if temp_err==1
	drop temp*
lab var hhinc_pypost_eq "HH income(prev. year, post, equivalized) [UK]"
...
UK original scale labels --> New CPF 2.0 scale labels:
======hlstat 
  Excelent --> Excellent 1
  Good --> Very good 2
  Fair --> Good 3
  Poor --> Fair 4
  Very poor --> Poor 5
...
label data "CPF_UK v2.0"	 	 
save "${ukhls_out}/uk_02_CPF.dta", replace  	

* Close log
display "Completed UKHLS data harmonization at $S_TIME"
log close
