...
Author:  Konrad Turek
Date:    06.2025
Input:   us_01.dta (combined person-household file)
Output:  us_02.dta (harmonized dataset)
===============================================================================
*/
/* NOTE:
...
* Log
capture log close 
log using "${psid_out}/us_02_harmonize.log", replace
display "Starting PSID data harmonization at $S_TIME"

* Configuration
global start_year = 1968
global waves_n = "${psid_w}"
global data_path = "${psid_out}"
global output_path = "${psid_out}"
...
* Fill age and yborn if missing	
	...
	* Correct yborn if not consistent values of yborn across waves
...
**--------------------------------------
/*NOTE:
- there are 2 items (eduy_f, eduyBH) which  differ - read description of the variables 
- eduy_f is for resp., eduyBH for HEAD
- we fill MV within individuals with information from other waves 
  (the procedure can be modified) 
- it is recommended to clean the final variable (e.g. jumping values within individual)
*/
...
**--------------------------------------
*  
//	kids_birth 	// # LIVE BIRTHS TO THIS INDIVIDUAL  (not useful - constant, only latest reported wave)
...
*################################################################################
*# 
*#	Labour market situation: working, employment status
*# 
*################################################################################
...
**** Crosswalks' codes are very long, so they were stored externally. 
...
gen isco08_4=.
	...
	replace isco08_4=occupD_isco08  if intyear>2001 & intyear<2017 & href==1
	replace isco08_4=occupDS_isco08 if intyear>2001 & intyear<2017 & href==2

	replace isco08_4=occupE_isco08  if intyear>=2017 & href==1
	replace isco08_4=occupES_isco08 if intyear>=2017 & href==2
...
***	D (2003-2015: 3 dig Census 2010) 	  
* 'if wave' left for compatibility with previous code, althoug it is no longer required 
...
*** E 2017+ (4 digit Census 2012):	
* 'if wave' left for compatibility with previous code, althoug it is no longer required 	
foreach var in  industE industES { 			
...
gen indust`n'=.
	...
	replace indust`n'=temp_industE_`n'b  if wave>=40 & href==1
	replace indust`n'=temp_industES_`n'b if wave>=40 & href==2
}
...
*################################################################################
*# 
*#	Sector, size of organization, hours worked and other 
*# 
*################################################################################
...
recode publicA publicAS (1=1)(5=0)(8 9=-1)(0=-3) (.=-1) ///
	   if intyear>=1975 & intyear<=1982, gen(temp_publicA1 temp_publicAS1)
recode publicA publicAS (1/3=1)(4 7=0)(8 9=-1)(0=-3) (.=-1) ///
	   if intyear>=1983 & intyear<=2001, gen(temp_publicA2 temp_publicAS2)
recode publicB1_ publicB2_ (1/3=1)(4 7=0)(8 9=-1)(0=-3) (.=-1) ///
...
* 
/*Notes: 
- create set of categories if harmonization possible
*/
...
replace selfemp=. if href==2 & intyear<1976
	...
	replace selfemp=1 if href==1 & (selfempBMJa==3)  & intyear>=2003 & intyear<2017
	replace selfemp=1 if href==1 & (selfempBMJb==3)  & intyear>=2017

	replace selfemp=1 if href==2 & (selfempAS==3)   & intyear>=1976 & intyear<2003
	replace selfemp=1 if href==2 & (selfempBMJSa==3) & intyear>=2003 & intyear<2017
	replace selfemp=1 if href==2 & (selfempBMJSb==3) & intyear>=2017
...
- Info about farmers in 'farmer' or ISCO (but not precise) 
*/
...
// 		G32.== (who receives? HEAD=? / SPOUSE=?) // included in ret_pensC
...
**-------------------------------------- 
// PROBLEM: no pension info 93-03

/* retf - Criteria if oldpens available:
...
recode href (1 2=0), gen(exporg)
	...
	lab var exporg "Experience in organization"
...
* all jobs  
	...
	gen 	incjobs_pyg=.
	replace incjobs_pyg=incj_yH  	if href==1
	replace incjobs_pyg=incj_yS1_ 	if href==2 & intyear<1993
	replace incjobs_pyg=incj_yS2_	if href==2 & intyear>=1993
	
	recode incjobs_pyg (9999999=.)
...
* main job
/* NOTE:
Although some data cleaning is done below, some more exploration and cleaning is advised before using
...
**--------------------------------------	
* hhinc_pypre
* hhinc_pypost - //better indicator
/* NOTE: 
	- PSID provides values below zero (from 1994) - consider before using
*/

gen hhinc_pypost=hhinc_preALL
recode hhinc_pypost (9999998 9999999=-1)
...
*################################################################################
*# 
*#	Labour market: training, work-education link
*# 
*################################################################################
...
1 "Religious" ///
3 "Not or OTHER (US 1970-1984)" ///
-1 "MV general" ///
-8 "Question not asked in survey"

recode rel_pref_H1 (0=3) (1/9=1) (else=-1), gen(temp_rel_A)
recode rel_pref_H2 (0=0) (1/25=1) (else=-1), gen(temp_rel_B)
recode rel_pref_H3 (0=0) (1/97=1) (else=-1), gen(temp_rel_C)
recode rel_pref_H4 (1 2 3=0) (4/97=1) (else=-1), gen(temp_rel_D)

recode rel_pref_S1 (0=3) (1/9=1) (else=-1), gen(temp_relS_A)
recode rel_pref_S2 (0=0) (1/25=1) (else=-1), gen(temp_relS_B)
recode rel_pref_S3 (0=0) (1/97=1) (else=-1), gen(temp_relS_C)
recode rel_pref_S4 (1 2 3=0) (4/97=1) (else=-1), gen(temp_relS_D)

*** Head and spouse
gen relig=.

*** Head
/* NOTE for 1970-1984: No/Other is combined into 3.
Value 1 is identified but missing some Other religions. 
*/
replace relig=temp_rel_A if inrange(wavey, 1970, 1984) & href==1
//replace relig=-8 if inrange(wavey, 1970, 1984)
replace relig=temp_rel_B if inrange(wavey, 1985, 1993) & href==1
replace relig=temp_rel_C if inrange(wavey, 1994, 2017) & href==1
replace relig=temp_rel_D if inrange(wavey, 2019, 2023) & href==1
replace relig=-8 if wavey<1970

*** Spouse
replace relig=temp_relS_A if wavey==1976 & href==2	
//replace relig=-8 if inrange(wavey, 1970, 1984)
replace relig=temp_relS_B if inrange(wavey, 1985, 1993) & href==2
replace relig=temp_relS_C if inrange(wavey, 1994, 2017) & href==2
replace relig=temp_relS_D if inrange(wavey, 2019, 2023) & href==2

*** Fill missing if not asked
replace relig=-8 if wavey<1970 & href==1
...
incjobs_pyg incjob1_mg incjob1_hg hhinc_pypost*	///
srh5 disab disab2c 	///
w_ind*   xsqnr	///
...
label data "CPF_USA v2.0"

save "${psid_out}/us_02.dta", replace  	

* Log close
display "Completed PSID data harmonization at $S_TIME"
log close
