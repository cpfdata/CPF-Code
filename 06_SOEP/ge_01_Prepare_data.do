...
===============================================================================
Purpose: Prepare and combine SOEP panel data files across all waves
Author:  Konrad Turek
Date:    06.2025
Input:   soep{w}p.dta, soep{w}h.dta files
Output:  ge_01.dta (combined person-household file)
===============================================================================
*/

* Log
capture log close 
log using "${soep_out}/ge_01_preparation.log", replace
display "Starting SOEP data preparation at $S_TIME"


/* 
================================================================================
SOEP Data Preparation Description - ge_01_Prepare_data.do
================================================================================
- The ge_01_Prepare_data.do file processes SOEP panel data by selecting relevant 
  variables from multiple specialized files and combining them into a single 
  harmonized dataset for CPF analysis.
- SOEP's modular structure requires careful variable selection before 
  merging due to the large number of available variables across specialized files.
- Becouse of the size and number of source SOEP data files, we first select 
  the variables that are included in the harmonization.

Step 1: Prepare Specific Files
------------------------------
Due to SOEP's large size and numerous source files, variables are first 
selected from each specialized file:
• ppathl.dta (Master File): Person identifiers, demographics, sample 
  information, migration background
• hpathl.dta (Household Path): Household identifiers and basic household 
  characteristics  
• pgen.dta (Generated Variables): Labor income, employment status, ISCO 
  codes, working time, education (ISCED, CASMIN)
• health.dta (Health Module): Physical/mental health scales (PCS/MCS), 
  BMI, health indicators (2002+ waves)
• pequiv.dta (CNEF Equivalents): Standardized income variables, weights, 
  household composition
• pl.dta (Person Questionnaire): Detailed work variables, satisfaction 
  measures, unemployment, retirement, disability
• pkal.dta (Calendar Data): Previous year employment history and status 
  transitions
• biobirth.dta (Birth Biography): Number of children
• biol.dta (Life Course): Parental information, mother tongue  
• bioparen.dta (Parental Background): Parents' education, occupation, 
  country of origin

Step 2: Merging Separate Files
------------------------------
• Starts with master person file (ppathl_1.dta)
• Merges household data using household ID and year
• Sequentially merges all person-level files using person ID and year
• Uses many-to-one merges for time-invariant biographical data
• Maintains comprehensive person-year structure

Step 3: Final Dataset Creation
------------------------------
• Creates final integrated dataset: ge_01.dta
• Removes temporary working files
• Provides foundation for subsequent harmonization steps

================================================================================
ADDING NEW VARIABLES:
- If users want to add new variables from the source data files, 
  they should add them to the specific files. 
- Code ge_01_Prep_data is where new raw variables from the original SOEP data 
  files should be added. Users must identify the specific original data file and 
  variables and add the name(s) in an appropriate place in the syntax.
- SEE Manual for more details and an example.
...
global start_year = 1984
global waves_n = "${soep_w}"
global data_path = "${soep_in}"
global output_path = "${soep_out}"
...
*################################################################################
/*
The code below prepares the specific files one by one.
*/

*================================================================================
*	Prepare the master file - based on ppathl.dta	
*================================================================================
...
imonth   /// previously pgmonth
pgpartnr	/// pgpartz less accurate that partner from ppath
pgfamstd
...
kal2d01_* kal2d02		/// receipt of pension / retirement income in the previous year
...
keep  pid cid spelltyp  // hhnr is missing in v40
*
use "${soep_in}/biocouply.dta", clear
keep  spelltyp pid cid 
*/
...
****	Check the number of variables and observations
disp "vars: " c(k) "   N: " _N	
...
local files : dir "${soep_out_work}" files "*.dta"
foreach f of local files {
    erase "${soep_out_work}/`f'"
}

* Log close
display "SOEP data preparation completed at $S_TIME"
log close
