...
* Log
capture log close 
log using "${soep_out}/ge_02_harmonize.log", replace
display "Starting SOEP data harmonization at $S_TIME"

* Configuration
global start_year = 1984
global waves_n = "${soep_w}"
global data_path = "${soep_out}"
...
gen intmonth=imonth // previously 'pgmonth'
recode intmonth (-5=-1)
...
*################################################################################
*# 
*# Socio-demographic basic variables: age, gender, birth year
*# 
*################################################################################
**--------------------------------------
** Demographic
...
* Gender
recode sex (1=0) (2=1) (-3 -1=2) (else=.), gen(female)
...
*################################################################################
**--------------------------------------
** Primary partnership status  (from CNEF variable) 	 
**--------------------------------------
...
*# 
*################################################################################
* NOTE: Employment status is defined in the later sections
...
*** isco88_4
clonevar isco08_4 = e11105 
recode isco08_4 (-2 -8=-3) (0=-1) 



capture lab copy e11105 isco08_4
...
* Major groups 
recode e11107 (0 -2=-3)(1/15=1)(16/26 32=2)(27/31 33=3) (99 -8 .=-1), gen(indust1)
...
* Submajor groups 
recode e11106 (0 -2=-3) (-8 .=-1), gen(indust2)
...
recode e11107 (0 -2 99=-3)(3=5)(4 =3)(5 6 7 8 11 12 13=4)(9 10 14 15=6) ///
			  ...
			  (27=13)(28=14)(29 30 31=15)(32=16)(33=12) (99 -8 .=-1), gen(indust3)
...
- plb0040 changed at some point to plb0040_h, plb0040_v1, plb0040_v2
*/
recode pgoeffd (1=1) (2=0) (-1=-1) (-2=-1), gen(public)
replace public=0 if plb0040_h==2 & public==-1
replace public=1 if plb0040_h==1 & public==-1
...
*################################################################################
*# 
*# Labour market situation: working, hours worked, employment status
*# 
*################################################################################
**--------------------------------------
** Currently working 
...
recode whweek (min/-1=-1)
recode whyear (min/-1=-1)
...
** overtime working  
**--------------------------------------
* pguebstd plb0193_* plb0196_h plb0197
...
*################################################################################
*# 
*# Labour market situation: self-employed, entrepreneur
*# 
*################################################################################
**--------------------------------------
...
*################################################################################
*# 
*# Labour market situation: retired
*# 
*################################################################################
...
replace retf=1  if (plb0022_h==9 | plb0022_h==7 | plb0022_h==5) &	 pglfs!=11 &	/// 
					  (kal1e01==1 | kal2d01_h==1)  &		/// received pension (maybe early)
					  age>=50						//  age 
...
*################################################################################
*# 
*# Labour market situation: employment status, full/part time, old-age pension, disability pension
*# 
*################################################################################
* Variables moved here to use some previous variables (e.g, retf and un_act) 
...
* recode 0 to MV for any remaining cases (only 1 case in CPF 2.0)
recode emplst5 (0=-1)
...
/*Notes: 
*/
recode plb0022_h (1=1) (2 4=2) (3 5/12=3) (min/-1 .=-1), gen(fptime_r)
...
*   
/*Note: 
- if kal1e01==1 | kal2d01_h==1
- do it only if possible to harmonize 
*/								  
recode kal1e01 (1=1) (2=0) (-5 -1=-1), gen (oldpens)
replace oldpens=1 if kal2d01_h==1
replace oldpens=0 if kal2d01_h==2 & oldpens!=1
replace oldpens=-1 if kal2d01_h<0 & oldpens!=1 & oldpens!=2
...
*################################################################################
*# 
*# Work history: labor market experience, experience in organization
*# 
*################################################################################
...
* main job
clonevar incjob1_pyg=ijob1
clonevar incjob1_mg=pglabgro 
clonevar incjob1_mn=pglabnet

	lab var incjob1_pyg  "Salary from main job (prev. year, gross)"
...
*/
clonevar hhinc_pypre=i11101 
clonevar hhinc_pypost=i11102 //better indicator
...
SOEP original scale labels --> New CPF 2.0 scale labels:
...
recode ple0036 (1=1)(2=0) (-1 -4=-2)(-2=-3)(-8 -5=-8), gen(chron)
...
*################################################################################
*# 
*# Other variables: training, work-edu link, job security
*# 
*################################################################################
**--------------------------------------
**   Training
...
recode plh0042 (1=1)(2 3=0)(-1=-2)(-2=-3)(-6 -5 -9=-8), gen(jsecu)
...
**--------------------------------------	
* iscogen mps88 = mps(isco08_4), from(isco08)
...
**--------------------------------------
/* 
Note: This recording might need adjustment 
...
*** Father    	   
*edu3
recode fsedu (1 2 8 =1) (3/5 7 9=2) (min/-1 6=-1), gen(fedu3)
recode fprofedu (7 8 9=1)  (1/4 10 12 13=2) (5 6 11=3)(else=-1)	,  gen(temp_fedu3)
...
*** Mother 
*edu3
recode msedu (1 2 8 =1) (3/5 7 9=2) (min/-1 6=-1),  gen(medu3)
recode mprofedu  (7 8 9=1)  (1/4 10 12 13=2) (5 6 11=3)(else=-1),  gen(temp_medu3)
...
*** Fill MV based on other waves 
	...
	bysort pid (wave): replace `p'edu3=`p'temp3 if (`p'edu3<0 | `p'edu3==.)  
	drop `p'temp3
	}
...
do "${Gdr_syntax}/06_SOEP/ge_02add_labels_COB.do" 
...
label data "CPF_Germany v2.0"	 
save "${soep_out}/ge_02_CPF.dta", replace  	


*** Log close
display "Ending GE 02 harmonization at $S_TIME"
log close
