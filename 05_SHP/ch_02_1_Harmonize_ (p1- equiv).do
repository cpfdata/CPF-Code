...
CPF Version 2.0 
SHP 
Syntax 02_1: Harmonize variables - CNEF file equiv	
===============================================================================
Purpose: Harmonize variables in SHP-CNEF file equiv
Author:  Konrad Turek
Date:    06.2025
Input:   ch_01_shpequivL.dta 
Output:  ch_02a_cnef.dta 
===============================================================================
*/
...
* Log
capture log close 
log using "${shp_out}/ch_02_1_preparation.log", replace
display "Starting SHP data preparation at $S_TIME"


**--------------------------------------
** Open merged shpequiv (CNEF) dataset
**-------------------------------------- 
use "${shp_out}/ch_01_shpequivL.dta", clear

* Configuration
global start_year = 1999
global waves_n = "${shp_w}"
global data_path = "${shp_out}"
global output_path = "${shp_out}"
...
* Gender
recode d11102ll (1=0) (2=1) (-3 3=2), gen(female)
...
*################################################################################	
*#	
*#	Labour market situation: work, occupation, industry, hours worked
*#	
*################################################################################
...
/*
i11110
Labour earnings include wages and salary from all employment including primary and secondary jobs, self-employment, plus income from bonuses, overtime and profit-sharing. 
The salary reported is a gross salary before deduction of payroll taxes for insurances (old age, invalidity, unemployment, accidents) as well as compulsory pension plans).
*/
clonevar incjobs_pyg=i11110

// 	lab var incjobs_yn "Individual Labor Earnings (All jobs, year, net)"
	lab var incjobs_pyg "Individual Labor Earnings (All jobs, year, gross)"
...
recode i11101 (-2 -3=-2), gen(hhinc_pypre)
...
SOEP original scale labels --> New CPF 2.0 scale labels:
 (1) Very well --> Excellent
 (2) Well --> Very good
 (3) So, so (Average) --> Good
 (4) Not very well --> Fair
 (5) Not well at all  --> Poor
*/
